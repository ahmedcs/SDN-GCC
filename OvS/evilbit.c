...
{
    int i,j;
    if(evil_enabled() && !evil_fail)//|| !evil_timerrun)
    {
        if(evil_timerrun)
        {
		...
		if(active_count>1)
	     ...
	     for(i=0; i<ethdevcount; i++)
			...
			if(virt_rate[i * EVIL_DEV_MAX + j] < eth_fair_rate[i] && virt_isactive[i * EVIL_DEV_MAX + j] && ktime_us_delta(ktime_get(), virt_lastevil[i * EVIL_DEV_MAX + j]) >= 2000)
			{
				...
				/*if(virt_lastincr[i * EVIL_DEV_MAX +j])
				{
				     virt_rate[i * EVIL_DEV_MAX + j] = MIN(eth_fair_rate[i], virt_rate[i * EVIL_DEV_MAX + j] + 1 );
				     printk(KERN_INFO "OpenVswitch [Rateincrease:%pI4]: fair rate is %d, old rate is %d, new rate is %d\n", &virtipaddress[j], eth_fair_rate[i], virt_lastrate[i * EVIL_DEV_MAX + j],  virt_rate[i * EVIL_DEV_MAX + j]);
				}*/
...
void evil_process(struct sk_buff *skb,  struct vport *inp , struct vport *outp, struct sw_flow_key *key)
{
    ...
    if (skb && in && out && !evil_fail)
    {
		...
		/*if(ip_header->protocol == IPPROTO_UDP)
                	virt_tokens[i * EVIL_DEV_MAX + j] = MIN(virt_bucket[i * EVIL_DEV_MAX + j],  virt_tokens[i * EVIL_DEV_MAX + j] + (10 >> 3) * delta_us);
		else
	                virt_tokens[i * EVIL_DEV_MAX + j] = MIN(virt_bucket[i * EVIL_DEV_MAX + j],  virt_tokens[i * EVIL_DEV_MAX + j] + (200 >> 3) * delta_us);*/
                //virt_tcptokens[i * EVIL_DEV_MAX + j] = MIN(virt_bucket[i * EVIL_DEV_MAX + j]>>tcpburst,  virt_tcptokens[i * EVIL_DEV_MAX + j] + (virt_rate[i * EVIL_DEV_MAX + j] >> 3) * delta_us);
		   ...
		   /*if(ip_header->protocol == IPPROTO_UDP)
			printk(KERN_INFO "OpenVswitch [%s:%pI4->%s:%pI4(%pI4)]: sending packet %d and rate is %d and  bucket %d and tokens %d\n", (const char*)in, &ip_header->saddr,(const char*)out, &ip_header->daddr, &virtipaddress[j], skb->len, virt_rate[i * EVIL_DEV_MAX + j], virt_bucket[i * EVIL_DEV_MAX + j], virt_tokens[i * EVIL_DEV_MAX + j] );*/
                }
		else
		{
		    ...
		    if(virt_rate[m * EVIL_DEV_MAX + n] > 1 + MIN_RATE)
		    {
			   ...
			   bool larger = (1 * virt_lastdeltaus[m * EVIL_DEV_MAX + n]) >= (virt_lastcount[m * EVIL_DEV_MAX + n] * delta_us);
			    //if(!virt_evildetected[m * EVIL_DEV_MAX + n] && jiffies_to_msecs(jiffies -virt_lastincr[m * EVIL_DEV_MAX +n]) > 1 || (virt_evildetected[m * EVIL_DEV_MAX + n] && jiffies_to_msecs(jiffies -virt_lastincr[m * EVIL_DEV_MAX +n]) >=1))
			    if(larger || delta_us >= 3000)
			    {
...
void init_evil(void)
{
    ...
    printk(KERN_INFO "OpenVswitch Init: evil_enable: %d, rate: %d, gsosize %d\n", evil_enabled(), rate, gsosize);
    return;
}
...
void cleanup_evil(void)
{
    ...
    printk(KERN_INFO "OpenVswitch: Stop EVILBIT \n");
