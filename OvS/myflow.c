...
bool is_ecn(const struct sk_buff *skb, u8 tos)
{
    ...
    if(flow)
    {
        ...
        /*if(flow->in_packet_count==0)
            flow->in_packet_count=1;
        if(flow->in_packet_count <  flow->ecn_packet_count)
            flow->ecn_packet_count = flow->in_packet_count;
        flow->ecn_alpha = flow->ecn_alpha - (flow->ecn_alpha >> g) + ((flow->ecn_packet_count << (10-g)) / flow->in_packet_count);
        if(flow->ecn_alpha > 1024)
            flow->ecn_alpha=1024;*/
...
bool is_evil(const struct sk_buff *skb, u16 fragoff)
{
    ...
    if(flow)
    {
	    ...
	    flow->evil_lastfeedback = jiffies;
            ...
            if(flow->old_out_packet_count <  flow->evil_packet_count)
            	flow->evil_packet_count=flow->old_out_packet_count;
            flow->evil_alpha = flow->evil_alpha - (flow->evil_alpha >> g) + ((flow->evil_packet_count << (10-g)) / flow->out_packet_count);
        ...
        //ovs_track_used(flow, skb, false, is_evil_pkt);
        /*if(evil_count>0)
        {
                   flow->evil_packet_count += evil_count;
        }*/
        /*if(!is_track_pkt)
        	ovs_track_used(flow, skb, false);
        else
        	ovs_track_used(flow, skb, true);*/
...
void ovs_track_used(struct track_flow *flow, const struct sk_buff *skb, bool is_ecn, bool is_evil)
{
    ...
    if(is_evil)
    {
        ...
        //flow->capacity = MIN(bucket_capacity, MAX(bytes_interval, bucket_capacity - ((bucket_capacity * flow->old_alpha) >> 10)));
...
void init_flow(struct track_flow *flow)
{
    ...
    flow->evil_byte_count = 0;
    //spin_unlock(&flow->lock);
    //flow->tokens= bucket_capacity;
    //flow->capacity = bucket_capacity;
    //flow->fill_rate= bytes_interval;
    //flow->time_ns = ktime_to_ns(ktime_get());
    //flow->q = vmalloc(sizeof(struct PacketQueue));
    //Init_PacketQueue(&flow->q);
    clear_stats(flow);
